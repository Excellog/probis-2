<?php
  ...
  //   $cek_user = checkexist($_POST['username'],$_POST['password']);
  //   if ($cek_user == 0) {
  //     echo "<script>alert('User ini tidak terdaftar!!!')</script>";
  //   }else if ($cek_user == 1) {
    ...
    $var = exequtequery($mysqli,"Select * from pembeli");
      foreach ($var as $key => $value) {
          if ($value["username"] == $_POST["username"]) {
              $cek = 1;
              if ($value["password"][$key] != $_POST["password"]) {
                  echo "<script>alert('password salah')</script>";
              }
          }
      }
    if ($cek == 0) {
      echo "<script>alert('User ini tidak ada !!!!!!!!!!!!!!')</script>";
    }
  }
?>
